...
## 环境配备
1. **api key配备**：请在该项目下的[.env](.env)文件下配备您需要调用的模型api key。
2. **依赖安装**：运行[requirements.txt](requirements.txt)文件，`pip install -r requirements.txt`进行所需依赖下载。
...
> 请注意该文件中的调用路径，更换成自己实际的项目文件路径。建议使用绝对路径。
